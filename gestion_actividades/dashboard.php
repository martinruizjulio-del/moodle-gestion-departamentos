...
echo local_ga_dash_card('4. Informes de horas', 'Consultar horas reconocidas por alumno y revisar acumulados Tipo A y Tipo B.', new moodle_url('/local/gestion_actividades/hours_report.php'), 'Ver informe de horas', 'btn btn-secondary');
echo local_ga_dash_card('5. Archivo', 'Consultar talleres terminados o archivados sin recargar la vista principal.', new moodle_url('/local/gestion_actividades/archive.php'), 'Abrir archivo', 'btn btn-secondary');
echo local_ga_dash_card('6. Plantillas y permisos', 'Editar plantillas de certificados, portada del portafolio y usuarios autorizados.', new moodle_url('/local/gestion_actividades/portfolio_cover_template.php'), 'Editar portada PDF', 'btn btn-secondary');
echo html_writer::end_div();
...
echo html_writer::link(new moodle_url('/local/gestion_actividades/portfolio_pdf_all.php', ['sesskey' => sesskey()]), 'Descargar todos los portafolios', ['class' => 'btn btn-primary mb-2 mr-1']);
